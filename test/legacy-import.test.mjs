...
await check('import: settings.update recovers within the retry budget → imported (boot-time lock contention)', async () => {
...
await check('import: no settings seam → warn (not silent), no marker, nothing read or written', async () => {
  const t = boot()
  try {
    writeLegacyDoc(t.home, 'settings.yaml.imported', LEGACY_DOC)
