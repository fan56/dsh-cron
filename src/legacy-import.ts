 ...
 * by "section name = entry id". This plugin's old section was named `cron`,
 * NOT `dsh-cron` — the host's one-shot import silently drops it. This module
 * recovers it once:
 *
 *   1. read `<home>/settings.yaml.imported` (preferred) →
 *      `<home>/settings.yaml` (fallback);
 *   2. flat-parse the `cron:` section (scalars only, conservative);
 *   3. keep keys whose legacy value differs from the current effective value;
 *   4. `settings.update('dsh-cron', patch)` → on success write the audit
 *      marker `<home>/storages/dsh-cron/legacy-import.json`; on failure warn
 *      and write nothing (the next boot retries);
 *   5. marker present → the whole migration is skipped (idempotent — a later
 *      manual edit of the imported file must not resurrect old values).
 *
...
export interface LegacyImportInput {
	 ...
	 * always times out (2026-09-25 incident: the topics-memory sibling failed
	 * EVERY boot and its migration never converged). Defaults to 5 attempts
	 * 2s apart; a permanently orphaned lock (a SIGKILLed host never releases
	 * it — orphan recovery is an operator action) still exhausts the budget
	 * and stays retry-worthy. Tests shrink this to keep the suite fast.
	 */
...
export type LegacyImportOutcome =
	...
	| 'no-section' // legacy file exists without a `cron:` section, marker written
 ...
 * unquoted; a single-line quoted flow-JSON value (`'["a","b"]'`) is
 * JSON-parsed after unquoting — a malformed one stays a string. Unquoted
 * flow collections are kept as strings (conservative: this is not a full
 * YAML parser). Empty never reaches here (the line parser skips value-less
 * keys — they open a nested block).
 */
...
export async function runLegacySettingsImport(input: LegacyImportInput): Promise<LegacyImportResult> {
	...
	// Not silent: a quiet exit here is unobservable in a boot log.
	...
	if (present.length === 0) {
		writeMarker(markerPath, { at: new Date().toISOString(), outcome: 'no-section', source: source, imported, skipped })
	...
	if (Object.keys(imported).length === 0) {
		writeMarker(markerPath, { at: new Date().toISOString(), outcome: 'no-op', source: source, imported, skipped })
	...
	writeMarker(markerPath, { at: new Date().toISOString(), outcome: 'imported', source: source, imported, skipped })
