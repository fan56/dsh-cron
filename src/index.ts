...
export async function apply(ctx: Context, config: CronRuntimeConfig): Promise<void> {
  ...
  // real boot can run apply() before the settings service mounts, and the
  // direct read then silently no-settings'd the import away every boot. The
  // inject callback fires the moment the service exists (immediately when it
  // already does — fake-ctx tests stay synchronous), fire-and-forget so
